...
if(!empty($_GET)) {
	...
	if(!empty($_GET["dni"])) {
		...
		$nombre = $_GET["nombre"];
		$fullname = str_replace(' ', '', $nombre);
		$sql = $insQueryString . " WHERE (replace(apellido_benef || nombre_benef, ' ', '') ILIKE '%$fullname%' or apellido_benef ILIKE '%$nombre%' or nombre_benef ILIKE '%$nombre%') ";
