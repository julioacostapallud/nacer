	...
	if ($estado_env == 'n' and $trans_tip == 'A') {
 	...
 	$dias=calculo_dias(fecha($result->fields['$fecha_inscripcion']));
   	$query = "UPDATE uad.beneficiarios SET 
            ...
            fecha_carga='$fecha_carga', usuario_carga=upper('$usuario'),
            nombre_benef=upper('$nombre'), apellido_benef=upper('$apellido'), numero_doc='$num_doc', fecha_nacimiento_benef='$fecha_nac',
            clase_documento_benef=upper('$clase_doc'),tipo_documento=upper('$tipo_doc'), pais_nac=upper('$paisn'),
...
if ($_POST['guardar']=="Guardar Planilla"){
		$sql1 = "SELECT * FROM uad.beneficiarios	  
				WHERE numero_doc='$num_doc' and nombre_benef='$nombre' and tipo_documento='$tipo_doc' and clase_documento_benef='$clase_doc'";
		...
		if ($res_extra1->recordcount()>0){
			...
			if(($id_categoria=='5') && ($sexo=='M')) { 
				...
				$responsable=$res_extra1->fields['responsable'];
			...
			}else// Mayor de 18 a�os Femenino muesta la informaci�n de embarazo.
		 		if (($id_categoria=='6') && ($sexo=='F')){
					...
					$semanas_embarazo=$res_extra1->fields['semanas_embarazo'];
					$fecha_probable_parto=Fecha($res_extra1->fields['fecha_probable_parto']);		
		 		}else // Mayor de 18 a�os Masuclino no muestra la informaci�n de embarazo.
		 		if ($sexo=='M') {
					$embarazada=none;
					$datos_resp=none;
					$mva1=none;
...
//-------------------comienza el insert del nuevo beneficiario--------------------
	...
	$id_planilla_clave= substr(str_pad($id_planilla, 6, '0', STR_PAD_LEFT), -6);
...
// Chequeamos que el campo A�o Mayor Nivel Tutor No sea ni NULL ni Vacio
if(($anio_mayor_nivel === "") or ($anio_mayor_nivel === NULL)){
	...
	$dias=calculo_dias(fecha($result->fields['$fecha_inscripcion']));
			...
			upper('$provincia_nac'),upper('$localidad_proc'),upper('$paisn'),upper('$indigena'),$id_tribu,$id_lengua,upper('$alfabeta'),upper('$estudios'),'$anio_mayor_nivel',
		 	upper('$menor_embarazada'), '$fecha_diagnostico_embarazo','$semanas_embarazo', '$fecha_probable_parto', '$fecha_efectiva_parto', '$fum', '$scoreriesgo',
    		...
    		if ($res_extra->recordcount()>0) {
    			...
    			$accion="INFO-La inscripci�n ha sido realizada correctamente. El benficiario esta en el PUCO.";       	
    			}
    		if ($res_extra->recordcount()==0){
    			sql($query, "Error al insertar la Planilla") or fin_pagina();
...
if ($_POST['b']=="b"){
	...
	$estatinas=$res_extra1->fields['estatinas'];
	$score_riego=$res_extra1->fields['score_riego'];
...
if($id_planilla){ ;
	...
	//datos personales del beneficiario
			...
			$tipo_doc=trim($resultado->fields['tipo_documento']); 
	...
	//datos de convivencia	
		...
		// toma el responsable del ni�o en el caso que conviva con alguien.
			...
			//}
			$scoreriesgo=$resultado->fields['score_riesgo'];
			...
			$estatinas=$resultado->fields['estatinas'];
...
if(($id_categoria=='') && ($edad == '')){
	...
	$menor_embarazada=none;
} elseif (($id_categoria == '6') && ($sexo=='F')){ //Femenino mayor de 10 a�os, pregunta si esta o no embarazada para mostrar la informaci�n de embarazo.
			...
			if ($menor_embarazada =='S'){
				...
				$embarazada=none;	
				...
				}elseif (($id_categoria=='5') && ($sexo=='M')) { // Masculino menor de 19 a�os, muestra la informaci�n de menor vive con adulto y no la de embarazo
					...
					} elseif (($id_categoria=='6') && ($sexo=='M')) {// Masculino mayor de 19 a�os, no muesta la informaci�n de embarazo ni tampoco la de menor vive con adulto.
...
<script>
	...
	$(document).ready(function () {
		...
		var maxper1 = document.all.hidden_fecha_maxperiodo.value;
		var array_maxper1 = maxper1.split("-");
		var diam1 = parseInt(array_maxper1[2], 10);
		var mesm1 = parseInt(array_maxper1[1], 10);
		var anom1 = parseInt(array_maxper1[0], 10);
		var minInsDate = new Date();
		minInsDate.setFullYear(anom1, mesm1 - 1, diam1);	
		$(".date-input-ins").datepicker({ 
			maxDate: "0",
			minDate: new Date(Date.parse(minInsDate)),
			changeMonth: true,
			changeYear: true,
			onSelect: function() { this.focus(); }
		});
...
window.open(url_map)
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data) {
					...
					$("#modal_message").html("<strong>Atencion!</strong> No se encontraron beneficiarios para el documento ingresado.<br/>Por favor, realice la inscripci�n correspondiente.");
	...
	function esNacimientoAnteriorInscripcion() {
		if (fechaNacAct() <= '-1') {
			//alert("La fecha de inscripcion no puede ser mayor a la fecha de nacimiento");
	...
	//Funcion para verificar si una fecha de inscripcion es v�lida
	function esFechaInsValida(fecha) {
		...
		maxper = document.all.hidden_fecha_maxperiodo.value;
		var array_maxper = maxper.split("-");
		var diam = parseInt(array_maxper[2], 10);
		var mesm = parseInt(array_maxper[1], 10);
		var anom = parseInt(array_maxper[0], 10);

		var y = new Date();
		y.setFullYear(anom, mesm - 1, diam);
	...
	function control_nuevos() {
		...
		if (document.getElementById('id_categoria').value== -1){
			...
			cambiar_patalla();
			alert("Debe ingresar los datos del responsable");
			document.all.menor_convive_con_adulto.focus();
			return false;
		}
		// Validaci�n Menores de 10 a�os
		if(ed <= 9) {
		...
		if(document.all.clase_doc.value == 'A') {
			if(ed < 1) {
				...
				alert("No se puede inscribir un ni�o mayor de un a�o con DNI ajeno");
				document.all.num_doc.focus();
				return false;
			}
		} // FIN
		...
		//Control de la fecha de inscripci�n	
		...
		if((esFechaFutura(document.all.fecha_inscripcion)) == false) {
			alert("La fecha de inscripci�n no puede ser mayor que la actual");
			document.all.fecha_inscripcion.focus();
			return false;
		}
		if((esFechaInsValida(document.all.fecha_inscripcion) == false) && (document.all.tipo_transaccion.value != "M")) {
			alert("La fecha de inscripcion no puede ser menor que el mes cerrado");
			document.all.fecha_inscripcion.focus();
			return false;
		}
		...
		//Control de fecha de nacimiento anterior a la inscripcion
		if(esNacimientoAnteriorInscripcion() == false) {
			alert("La fecha de nacimiento no puede ser mayor que la fecha de inscripcion");
			document.all.fecha_nac.focus();
			return false;
		}
		//Control de efector
	...
	function cambiar_patalla() {
		// Masculino - Menor de 10 a�os edad no muestra la informaci�n de embarazo, muestra la informaci�n de menor vive con adulto 
		...
		// Masculino - Mayor de edad 10 a�os no muestra la informaci�n de embarazo, no muestra la informaci�n de menor vive con adulto. 
		...
		// Femenino - Mayor de 10 a�os puede o no estar embarazada,pregunta si esta o no 
	...
	function edad(Fecha) {
		...
		if(fechaNacAct() <= '-1') {
			//alert("La Fecha de Nacimiento no puede ser mayor al d�a de hoy");
			//document.all.fecha_nac.focus();
			return false;
		}
		//si es mayor de 10 a�os categoria 6
		if(ed >= 10) {
			document.getElementById('id_categoria').value = '6';
			document.getElementById("edades").value = ed;
			if(document.all.clase_doc.value == 'A') {
				alert("Mayor de 1 a�o, el documento debe ser Propio")
				document.all.clase_doc.focus();
			}
		}
		//si es menor de 10 a�os categoria 5
		if(ed <= 9) {
	...
	//Calcular el color segun el RCVG
	function calcularColor(valor)
	{
		if (valor == "" || valor == 0) {$('#rcvg').removeClass();
					$('#rcvg').text('No esta cargado');
					$('#rcvg').addClass('alert alert-light');
		};
		if (valor == 1) {$('#rcvg').removeClass();
						$('#rcvg').text('Menor a 10%');
						$('#rcvg').addClass('alert alert-success');
			};
		if (valor == 2) {$('#rcvg').removeClass();
						$('#rcvg').text('Entre 10 y 20%');
						$('#rcvg').addClass('alert alert-warning');
			};
		if (valor == 3) {$('#rcvg').removeClass();
						$('#rcvg').text('Entre 20 y 30%');
						$('#rcvg').addClass('alert alert-warning');
			};
		if (valor == 4) {$('#rcvg').removeClass();
						$('#rcvg').text('Entre 30 y 40%');
						//$('#rcvg').addClass('alert alert-danger');
						$('#rcvg').addClass('alert');
						$('#rcvg').css("background-color", "#ffe0b2");
			};
		if (valor == 5) {$('#rcvg').removeClass();
						$('#rcvg').text('Mayor a 40%');
						$('#rcvg').addClass('alert alert-danger');
			};
	}
	
	function handleClick(myElement) {
		var chk = $('#redes input:radio:checked').length;
		if ((chk >= 5) && ($('#hta').val().length > 0))
		{
			getRCVG();
		};
	}
	...
	//Validar TA
	function validateTension() {
		var minSis = 50;
		var maxSis = 300;
		var minDia = 40;
		var maxDia = 150;
		var tension = document.all.hta.value;
		//alert(tension);
		var valid;
		var re = /^\s*\d{2,3}[\/]\d{2,3}\s*$/;
		if (re.test(tension)) {
			var sis = parseInt(tension.split('/')[0]);
			var dia = parseInt(tension.split('/')[1]);
			valid = (sis >= minSis && sis <= maxSis && dia >= minDia && dia <= maxDia);
			//alert(sis + " - " + dia + "Valid: " + valid);
		} else {
			valid = false;
		}
		if (!valid) {
			alert("Tensi�n Arterial inv�lida");
	...
	function getRCVG() {
		...
		var fumador = $("input[name='fumador']:checked").val();
		var hta = parseInt(tension.split('/')[0]);
		var edad = document.all.edades.value;
		//alert("sexo:" + sexo + "diabetes:" + diabetes + "fumador:" + fumador + "hta:" + hta + "edad:" + edad);
		$.ajax({
			type: 'GET',
			url: '../../services/riesgocardiovascular.php',
			data: "sexo=" + sexo + "&diabetes=" + diabetes + "&fumador=" + fumador + "&hta=" + hta + "&edad=" + edad,
			async: false,
			success: function (data) {
				if(data) {
					$.each(data, function (index, itemData) {
						$('input[name="scoreriesgo"]').val(itemData.calcularscoreriesgo);
						//alert($('input[name="scoreriesgo"]').val());
						$('#scoreriesgo').val(itemData.calcularscoreriesgo);
						//alert($('#scoreriesgo').val());
						//alert(itemData.calcularscoreriesgo);
						calcularColor(itemData.calcularscoreriesgo);
					});
				} else {
					$('#scoreriesgo').val("0");
				}
...
<form name='form1' action='ins_admin.php' accept-charset="utf-8" method='POST'>
	...
	<?
	$query2 = "SELECT cast(cast(MAX(PERIODO)|| '01' as date) + interval '2 month' as date) periodo FROM nacer.smiprocesoafiliados";
	$f_res = $db->Execute($query2) or die($db->ErrorMsg());
	$maxPeriodo = $f_res->fields['periodo']
	?>
	<input type="hidden" value="<?=$maxPeriodo?>" id="hidden_fecha_maxperiodo">
	
	<p class="legend"><?= ($id_planilla)? "Editar Beneficiario <small>#" . $clave_beneficiario . "</small>" : "Nuevo Beneficiario" ?></p>

	<div class="alert">
		<button type="button" class="close" data-dismiss="alert">�</button>
		<strong>Atenci�n!</strong> Al ingresar valores num�ricos no utilice separadores de miles.
	</div>
	...
	<div class="row-fluid">
		<div class="span12">
			...
			<select name="tipo_transaccion" onKeypress="buscar_combo(this);" onblur="borrar_buffer();" 
				...
				<option value='A' <?if ($tipo_transaccion=='A') echo "selected"?>>Inscripci�n</option>
				<option value='M'<?if ($tipo_transaccion=='M') echo "selected"?>>Modificaci�n</option>
			</select>	
		</div>
	</div>
	...
	<div class="row-fluid">
		<div class="span3">
			<label>�Pertenece a alg�n Pueblo Originario?</label>
			<label class="radio">
	...
	?>
		<div class="alert color-fieldset">
			...
			<p class="legend form-title">
				Alfabetizaci�n del Responsable
			</p>
			...
			<div class="row-fluid">
				...
				<div class="span3">
					<label>A�os en el Mayor Nivel:</label>
	...
	<div class="">
		...
		<input class="input-large date-input-ins" type=text onblur="esFechaValida(this); esFechaInsValida(this);" name=fecha_inscripcion id=fecha_inscripcion value='<?=$fecha_inscripcion;?>' size=15 onkeypress="return pulsar(event)" <?php if (($id_planilla) and ($tipo_transaccion == "M"))echo "disabled"?> >
	...
	<div id="redes" style="display:<?= $redes ?>;" >
		...
		<div class="row-fluid">
			<div class="span2">
				<label>Fumador:</label>
				<label class="radio">
					<input type="radio" name="fumador" value="S" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($fumador == "S") echo "checked" ;?> > SI
				</label>
				<label class="radio">
					<input type="radio" name="fumador" value="N" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($fumador == "N") echo "checked" ;?> > NO
				</label>
			</div>
			...
			<div class="span2">
				...
				<label class="radio">
					<input type="radio" name="diabetes" value="S" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($diabetes == "S") echo "checked" ;?> > SI
				</label>
				<label class="radio">
					<input type="radio" name="diabetes" value="N" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($diabetes == "N") echo "checked" ;?> > NO
				</label>
			</div>
			...
			<div class="span2">
				...
				<label class="radio">
					<input type="radio" name="infarto" value="S" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($infarto == "S") echo "checked" ;?> > SI
				</label>
				<label class="radio">
					<input type="radio" name="infarto" value="N" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($infarto == "N") echo "checked" ;?> > NO
				</label>
			</div>
			...
			<div class="span2">
				<label>ACV:</label>
				<label class="radio">
					<input type="radio" name="acv" value="S" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($acv == "S") echo "checked" ;?> > SI
				</label>
				<label class="radio">
					<input type="radio" name="acv" value="N" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($acv == "N") echo "checked" ;?> > NO
				</label>
			</div>
			
			<div class="span2">
				<label>Usa Estatinas:</label>
				<label class="radio">
					<input type="radio" name="estatinas" value="S" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($estatinas == "S") echo "checked" ;?> > SI
				</label>
				<label class="radio">
					<input type="radio" name="estatinas" value="N" onclick="handleClick(this);" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> <?php if($estatinas == "N") echo "checked" ;?> > NO
				</label>
			</div>
			
			<div class="span2">
				<label>HTA:</label>
				<input type="text" size="10" value='<?=$hta?>' name="hta" id="hta" onchange="validateTension();"
				<?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?>>
			</div>			
		</div>
				
		<div id="riesgo" class="row-fluid">
			<div class="span3">
				<label>Score de riesgo:</label>
				<input type="text" value='<?=$scoreriesgo?>' name="scoreriesgo" id="scoreriesgo" disabled>
			</div>
			<div class="span3">
				<br>
				<div id="rcvg" class="alert" role="alert">
	...
	<div class="row-fluid">
		<div class="row6">
			...
			<textarea class="input-xxlarge" name='obsgenerales' onkeypress="return pulsar(event)" <?php if (($id_planilla) and ($tipo_transaccion != "M"))echo "disabled"?> > <?=$obsgenerales;?></textarea>
		</div>
	</div>
	...
	?>
		<div class="form-actions">
			...
			<div class="alert alert-error">
				<strong>Atenci�n:</strong> Por favor, verifique todos los datos antes de guardar.
			</div>
