...
<form name=form1 action="ins_listado.php" method=POST>
	...
	<div class="row-fluid">
		...
		<a class ="btn btn-small btn-info " type='button'  href="http://programasumar.com.ar/consultas" target="_blank"> Consultar PUCO</a>
		</div>
		<div class="span4">
...
if ($_POST['generarnino']){
    	...
    	if (!$result1->EOF) {
	    	...
	    	while (!$result1->EOF) {
				...
				$contenido.=$id;	//	N�mero de identificaci�n de lengua
				$contenido.=chr(9);
	    			$id = $result1->fields['id_tribu'];
				if (is_numeric($id) == -1) { $id = 0;}
					//$tribu = str_replace(null,0,$result1->fields['id_tribu']);
				$contenido.=$id;	//	N�mero de tribu
				$contenido.=chr(9);
			...
			if ($result1->fields['tipo_transaccion']== 'M'){
				...
				$contenido.=chr(9);
				$contenido.=$fecha_fum;	//10	AAAA-MM-DD (A�o, Mes, D�a)
				$contenido.=chr(9);
				...
				$contenido.=chr(9);
				$contenido.=chr(0); //UbicacionLatitud (para ir georreferenciando el domicilio de cada beneficiario)
				$contenido.=chr(9);
				$contenido.=chr(0); //UbicacionLongitud (para ir georreferenciando el domicilio de cada beneficiario)
				$contenido.=chr(9);
				$contenido.=chr(0); //CUIEEfectorACargo (para ir completando con el efector que tenga a cargo el  beneficiario seg�n su ARS)
				$contenido.=chr(9);
				$contenido.=chr(0); //ID_MPI (para completar con el identificador que tengan en el Indice Maestro de Personas de la provincia)
				$contenido.=chr(9);
				$contenido.=chr(0); //ID_EquipoNuclear (para ir completando con el identificador del Equipo Nuclear que tiene a cargo cada beneficiario)
				$contenido.="\n";	
		   		$result1->MoveNext();
	    	}
