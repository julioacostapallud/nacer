...
class ArrayOfPermisos extends virtualArray {
	...
	function loadDB()
	{
		...
		switch (get_class($this->parent)) {
			...
			//console.log($this->parent->id_usuario)
		 	case parent_user:
				...
				// $q.="join permisos p using(id_permiso) where id_usuario=238 ";
	
				$q.="union ";
				$q.="select p.id_permiso,p.uname,p.desc,p.dir,p.id_tipo ";
				$q.="from usuarios u2 ";
				$q.="join grupos_usuarios gu on gu.id_usuario=u2.id_usuario AND u2.id_usuario={$this->parent->id_usuario} ";
				// $q.="join grupos_usuarios gu on gu.id_usuario=u2.id_usuario AND u2.id_usuario=238 ";
